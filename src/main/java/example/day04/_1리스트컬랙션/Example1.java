...
public class Example1 {
    public static void main(String[] args) {
        ...
        //- ArrayList, Vactor, LinkedList 클래스
            //동기화 : 여러 스레드가 하나의 함수를 동시에 호출했을때 호출순서대로 해당 함수를 끝날때가지 점유 상태
            //ArrayList : [배열] 동기화 지원X, 주로 단일스레드 사용
            //Vactor : [배열] 동기화 지원o, 주로 멀티스레드 사용
    ...
    //순회
        //일반for문    : for(int i=0; i<리스트명size(); i++){ 실행문; }
        //향상된for문   : for(타입 반복변수명 : 리스트명){ 실행문; }
        //forEach       : 리스트명.forEach( 변수명 -> { 실행문; })
        ...
        System.out.println("=========");
        //2. 향상된 for문   for( 타입 반복변수명 : list / 배열){} : 리스트 내 순차적으로 하나씩 객체를 반복변수에 대입
        ...
        //3. forEach (반복변수 -> 실행문) : *컬랙션 프레임워크에서 제공하므로 list 에서만 쓸 수 있음(배열 불가능)*
